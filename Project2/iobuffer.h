...
{  public:
	...
	virtual int Write (ostream &, int deleteflag = 0) const = 0; // write a buffer to the stream

	// these are the direct access read and write operations
	virtual int DRead (istream &, int recref); // read specified record
	virtual int DWrite (ostream &, int recref, int deleteflag = 0) const; // write specified record
